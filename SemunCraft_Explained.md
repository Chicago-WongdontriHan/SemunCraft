...
- An **Elixir spring** (b6, h4) pays the same way: every turn that ends with a pawn of yours on one banks **a quarter of an Elixir** (`ELIXIR_RATE`, 0.25), with **no limit** on how much is banked. The tile pulses green with a **+0.25** badge, the Elixir line in the panel shows the rate beside the count, and the spring's sound plays for your own. Elixir used to be drawn by hand, a whole turn per point; now that it pays for four units it accrues the way Gold does, and what keeps it honest is the pawn itself — 1 HP, standing still on a square both sides know — and the time it takes: a Paladin's 3 Elixir is **twelve turns** of holding a spring (six with both), a Mage's 2 is eight, a Guardian's or a Siege Tower's 1 is four.
