...
- **Observation** (32 channels): own and enemy pieces by type (14); HP as a fraction and out of 5; bishop/mage mana; pawns that can still double-step; obstacles; board cells; cells the agent can see (all of them without fog; fog hides enemy pieces); own and enemy target locks (the locking piece and the locked square); own and enemy spawns left; turns taken out of the cap; whether merging and spawning are allowed; whether the turn order is classic; and whether the agent moves first each round (White in the classic order, where the two sides don't take their turns the same way). The Mage, Paladin and Guardian came after these networks were trained, so each shows up on the channel of the existing type nearest its own shape (queen, knight and rook respectively, `TYPE_ALIAS` in `rl/encoding.js`) until they are trained again with channels of their own; `fortify`, `order` and `meteor` are left out of the action map (`legalMap`) the same way.
...
- **Use:** `chooseAction(state)` for `state.turn`, `playTurn(state)` until the turn passes (like `botTurn`), `evaluate(state, colour)`. In the worker, `opponent: "scripted"` plays either colour in either turn order. It is deterministic (a position-hash tie-break instead of the game's random stream) and takes about 2 ms a decision (0.5 ms before it gave orders).
