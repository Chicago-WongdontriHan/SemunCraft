...
- The minimap in the right panel shows the whole map, your fog, and a box around the part you are looking at. On phones, where that panel is hidden, the same map appears in the board's bottom-left corner while you are zoomed in. Tapping either one jumps the view there.
...
| **Jungle** | Palms (🌴) and temple ruins (🏛) | Monkey (🐒, 2HP) | Snake (🐍, 1HP) | Patches of undergrowth (🌿) hide the pieces standing in them (below) |
...
- **Resources** (`renderResources` in `js/ui.js`): **Coin** is what the King spends to spawn pawns and **Elixir** is the army's magic. Neither is a pool the rules keep yet, so the panel reads them off what the game already tracks: Coin is the spawn allowance (8 to start, one more every 6 turns, which is why the Spawn button no longer prints a count) and Elixir is the mana on that side's bishops. In AI vs AI both sides' counters are shown.
...
- **Mobile**: In portrait, the resource counters become a strip above the board and the buttons wrap into finger-sized rows below it (unit cards, minimap and hint are hidden). The status line wraps, and the board keeps its full size when zoomed in: the pan arrows are hidden (drag the board instead) and the corner map shows where the view is. A double tap never zooms the page (the board has its own pinch zoom). On short landscape screens the side panels shrink and the layout is centered. Touch devices get touch wording (tap a siege tower twice instead of right-clicking).
