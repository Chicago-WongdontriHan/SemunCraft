...
| **Siege Tower** | 🏰 | 4 | **One square, ordered a turn ahead** | Cardinal up to **5 squares, 2 damage, lobbed clean over everything** — obstacles and pieces alike — and the **shell bursts**: every piece on the 8 squares around the one it hit takes **1**, friend or foe alike (never the tower itself), so a hit catches the whole 3×3, 2 in the middle and 1 around it (`siegeSplash` in `js/combat.js` and `js/engine.js`; an ordered strike bursts the same way). It never steps anywhere of its own accord: a move is given as a **delayed order** to an adjacent square (`orderTargets` in `js/actions.js`), it rolls there at the head of the next turn, and **it does not fire on the turn it moves** (`runOrders` marks it `rolled`, the attacks that turn pass it over, and the next `turnUpkeep` clears the mark). Right-click it (or tap it twice) to un-siege back into two rooks, each with the tower's current HP (one rook if no tile next to it is free). |
