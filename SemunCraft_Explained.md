...
- **What it leaves out:** campaign objectives, delayed orders, Scry, `unsiege`, heal-locks, target locks (except the Paladin's, which fires on nothing else), and moving its King.
- **Use:** `chooseAction(state)` for `state.turn`, `playTurn(state)` until the turn passes (like `botTurn`), `evaluate(state, colour)`. In the worker, `opponent: "scripted"` plays either colour in either turn order. It is deterministic (a position-hash tie-break instead of the game's random stream) and takes about half a millisecond a decision.
- **Strength:** 24 of 24 against a random player in both turn orders and as either colour; against the old built-in AI as White in the classic order, about 37 wins in 40 against Easy and 38 in 40 against Hard, with 1–3 Paladins a game (the Elixir sink; Mage, Guardian and Siege are rare). `node tests/scripted.test.js` checks legality and repeatability, a 90% win rate against a random player, no more losses than wins against the built-in AI, that it spawns, holds tiles and builds Elixir-tier units, and speed.
