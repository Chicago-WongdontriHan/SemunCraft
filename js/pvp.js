...
function broadcastState(winner){ if(!pvpActive||!conn||!conn.open)return; conn.send(JSON.stringify({type:'state',pieces,turn,over,winner:winner||null,wt:whiteTargets,bt:blackTargets,blf:blackLastFrom,blt:blackLastTo,tiles:tileData,theme:mapTheme,log:logLines,el:elixir,mn:mined})); }
...
function onPeerData(raw){
  ...
  if(msg.el)elixir=msg.el; if(msg.mn)mined=msg.mn;
