...
game.run(`function __snap(){return{board:pieces,tiles:tileData,over,turn,whiteTurnCount,blackTurnCount,scans,elixir,mineTurns,goldSpent,
...
function playClassic(opts,label,stats){
  ...
  for(let n=0;n<MAX_ACTIONS&&!s.over;n++){
    ...
    if(!check(label+': after action '+n+' '+q(a),o,{board:s.board,tiles:s.tiles,over:s.over,
      ...
      elixir:s.elixir,mineTurns:s.mineTurns,goldSpent:s.goldSpent,winner:s.winner}))return;
  }
}
